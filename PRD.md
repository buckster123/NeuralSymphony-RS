...
| **M3** | Sonus bridge (Dream voice) + opt-in taste write-back — *delivered 2026-08-04: `produce`/`ns_render_full` (distill → generate → poll → download, credits printed, dry-run free), `feedback`/`ns_feedback` (verdicts → scoped affective memories + graded procedure outcomes, `ns-internal` anti-Larsen filter, hard opt-in gate). Reference audio is blocked upstream: Sonus-RS has no upload/cover transport yet — demand filed in its notes; the PRD §2 seed waits there.* | A produced track from a memory cluster (armed with a key); feedback re-weights via cerebro's own reinforcement — the graph drifts, the mapping stays pure |
